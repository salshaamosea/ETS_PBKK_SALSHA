		...
		if ($this->kategori_produk_model->read()->num_rows() > 0) {
			foreach ($this->kategori_produk_model->read()->result() as $kategori_produk) {
				$data[] = array(
					...
					'action' => '<span class="nav-icon oi oi-trash"></span><button class="btn btn-sm btn-info" onclick="edit('.$kategori_produk->id.')">Ubah</button>
					<button class="btn btn-sm btn-warning" onclick="remove('.$kategori_produk->id.')">Hapus</button>'
				);
			}
