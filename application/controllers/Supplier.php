		...
		if ($this->supplier_model->read()->num_rows() > 0) {
			foreach ($this->supplier_model->read()->result() as $supplier) {
				$data[] = array(
					...
					'action' => '<button class="btn btn-sm btn-info" onclick="edit('.$supplier->id.')">Ubah</button> <span class="oi oi-trash"><button class="btn btn-sm btn-warning" onclick="remove('.$supplier->id.')">Hapus</button></span>'
				);
			}
		} else {
			$data = array();
